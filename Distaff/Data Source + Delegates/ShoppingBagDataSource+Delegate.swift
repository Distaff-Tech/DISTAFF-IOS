...
extension ShoppingBagVC : UITableViewDataSource,UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if indexPath.row == objShoppingBagVM.shoppingBagItems.count {
            ...
            cell?.lblShippingCost.text =  "$\(calculateTotalPrice().2)"
            ...
            cell?.lblServiceCharge.text = "$\(calculateTotalPrice().1)"
             cell?.lblServiceChargeTitle.text = "Taxes"
            ...
            cell?.callBackPostImageTapped = {
                ...
                for i in 0 ..< (self.objShoppingBagVM.shoppingBagItems[indexPath.row].post_image?.count ?? 0) {
                    let image = self.objShoppingBagVM.shoppingBagItems[indexPath.row].post_image?[i] ?? ""
